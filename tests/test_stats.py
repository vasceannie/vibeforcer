...
class TestParseTimestamp:
    def test_none_cutoff_never_skips(self) -> None:
        assert not _parse_timestamp("2026-01-01T00:00:00+00:00", None), "None cutoff must not skip"

    def test_before_cutoff_skips(self) -> None:
        cutoff = datetime(2026, 3, 1, tzinfo=timezone.utc)
        assert _parse_timestamp("2026-02-01T00:00:00+00:00", cutoff), "entry before cutoff must be skipped"

    def test_after_cutoff_keeps(self) -> None:
        cutoff = datetime(2026, 1, 1, tzinfo=timezone.utc)
        assert not _parse_timestamp("2026-06-01T00:00:00+00:00", cutoff), "entry after cutoff must be kept"

    def test_invalid_timestamp_keeps(self) -> None:
        cutoff = datetime(2026, 1, 1, tzinfo=timezone.utc)
        assert not _parse_timestamp("not-a-date", cutoff), "invalid timestamp must not be skipped"
...
class TestAnalyze:
    ...
    def test_counts_deny_decision(self) -> None:
        stats = analyze([self._entry()])
        decisions = dict(stats.get("by_decision", []))
    ...
    def test_context_only_not_counted_as_allow(self) -> None:
        ...
        stats = analyze([entry])
        decisions = dict(stats.get("by_decision", []))
        assert decisions.get("context", 0) == 1, "context-only findings must be counted as context"
        assert decisions.get("allow", 0) == 0, "context-only must not inflate allow count"
    ...
    def test_retry_patterns_detected(self) -> None:
        ...
        retries = stats.get("retry_patterns", [])
        assert len(retries) > 0, "repeated denies in same session must produce retry patterns"

    def test_daily_counts(self) -> None:
        stats = analyze([self._entry()])
        daily = dict(stats.get("daily_counts", []))
    ...
    def test_rule_examples_capped_at_three(self) -> None:
        ...
        assert len(examples.get("GIT-001", [])) <= 3, "rule examples must be capped at 3"
